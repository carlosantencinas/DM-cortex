...
export default function CharacterSheet() {
  ...
  return <section className="character-sheet dnd-sheet">
    <div className="character-topbar"><Link to="/personajes" className="back-link">← Mis personajes</Link><div className="character-actions"><button className="btn-secondary">Compartir</button><button className="btn-secondary">⋯</button></div></div>
    <div className="dnd-character-header"><div className="character-portrait">{(character.name || '?').slice(0, 1).toUpperCase()}</div><div className="character-title"><span className="eyebrow">PERSONAJE</span><h1>{character.name || 'Sin nombre'}</h1><p>{[character.race, character.class, character.subclass].filter(Boolean).join(' · ') || 'Configura tu personaje'}</p></div><div className="header-stat"><span>NIVEL</span><strong>{character.level || 1}</strong></div><div className="header-stat"><span>COMPETENCIA</span><strong>{signed(character.proficiencyBonus || 2)}</strong></div></div>
    ...
    {tab === 'summary' && <>
      <div className="dnd-combat-strip"><div><span>PV</span><strong>{hp.actual} / {hp.max}</strong><div className="hp-bar"><div className="hp-fill" style={{width:`${hpPct}%`}} /></div></div><div><span>CA</span><strong>{character.ac || 10}</strong></div><div><span>INICIATIVA</span><strong>{signed(character.initiative || 0)}</strong></div><div><span>VELOCIDAD</span><strong>{character.speed || 9} m</strong></div><div><span>PERCEPCIÓN</span><strong>{character.passivePerception || 10}</strong></div></div>
