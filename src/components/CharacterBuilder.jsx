...
export default function CharacterBuilder() {
  ...
  const finish = async () => { if (!data.name.trim()) return setError('Escribe el nombre del personaje.'); try { const id = await createCharacter(data); navigate(`/personaje/${id}`); } catch (e) { setError(e.message); } };
  ...
  return <section className="character-builder">
    ...
    <div className="builder-steps">{STEPS.map(([id,label],index)=><button key={id} className={index===step?'step-active':index<step?'step-done':''} onClick={()=>index<step&&setStep(index)}><b>{index+1}</b><span>{label}</span></button>)}</div>

    <div className="builder-panel">
      {step===0 && <><div className="section-title"><h2>Elige tu clase</h2><span className="muted">Tu clase define tus capacidades principales.</span></div><div className="option-grid">{CLASS_OPTIONS.map(option=><button key={option} className={`choice-card ${data.class===option?'choice-selected':''}`} onClick={()=>set({class:option})}><strong>{option}</strong><span>Ver características, competencias y opciones</span></button>)}</div></>}
      ...
      {step===2 && <><div className="section-title"><h2>Características</h2><span className="muted">Método: {ABILITY_GENERATION_OPTIONS.find(x=>x.value===data.abilityMethod)?.label}</span></div><label className="field builder-method"><span>Método de generación</span><select value={data.abilityMethod} onChange={e=>set({abilityMethod:e.target.value})}>{ABILITY_GENERATION_OPTIONS.map(x=><option key={x.value} value={x.value}>{x.label}</option>)}</select></label><div className="builder-stat-grid">{stats.map(([key,label])=><label className="builder-stat"><span>{label}</span><input type="number" min="1" max="30" value={data.stats[key]} onChange={e=>setStat(key,e.target.value)}/><b>{Math.floor((data.stats[key]-10)/2)>=0?'+':''}{Math.floor((data.stats[key]-10)/2)}</b></label>)}</div></>}
    ...
    <div className="builder-actions">{step>0?<button className="btn-secondary" onClick={()=>{setError('');setStep(value=>value-1)}}>← Atrás</button>:<span/>}{step<STEPS.length-1?<button className="btn-primary" onClick={next}>Continuar →</button>:<button className="btn-primary" onClick={finish}>Crear personaje</button>}</div>
  </section>;
}
