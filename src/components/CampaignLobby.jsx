...
export default function CampaignLobby() {
  ...
  if (campaign) return <section className="campaign-lobby">
  ...
  return <section className="campaign-lobby"><div className="eyebrow">DM CORTEX</div><h2>Tu mesa de juego</h2><p className="muted">Crea una campaña como DM o introduce el código que te dio tu Dungeon Master.</p><div className="lobby-columns"><div className="section"><h3>Crear campaña</h3><label className="field"><span>Nombre</span><input value={name} onChange={e=>setName(e.target.value)} placeholder="La Mina Perdida"/></label><label className="field"><span>Descripción</span><textarea rows="3" value={description} onChange={e=>setDescription(e.target.value)} placeholder="Una breve descripción de la aventura"/></label><button className="btn-primary full" onClick={async()=>{try{setError('');await createCampaign(name,{description,maxPlayers:8})}catch(e){setError(e.message)}}}>⚔ Crear campaña como DM</button></div><div className="section"><h3>Unirme a una campaña</h3><label className="field"><span>Código de invitación</span><input value={code} maxLength="6" onChange={e=>setCode(e.target.value.toUpperCase())} placeholder="ABC123"/></label><button className="btn-secondary full" onClick={async()=>{try{setError('');await joinCampaignByCode(code)}catch(e){setError(e.message)}}}>🎲 Unirme</button></div></div>{error&&<p className="error">{error}</p>}</section>;
}
